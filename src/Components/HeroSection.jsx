...
const HeroSection = () => {
  ...
  // Social links
  const socialLinks = [
    { icon: FaGithub, link: "https://github.com/Mj-Marjan" },
    { icon: FaLinkedin, link: "https://www.linkedin.com/in/md-marjan/" },
    { icon: FaTwitter, link: "https://x.com/MdMarjan2233" },
    { icon: FaFacebook, link: "https://www.facebook.com/mdmarjan.mdmarjan.18" },
  ];

  return (
    <section
      className="relative min-h-screen flex flex-col md:flex-row items-center justify-between px-6 md:px-10 text-white overflow-hidden"
      id="home"
    >
      ...
      >
        <motion.p
          ...
          className="text-[#64ffda] font-semibold text-sm md:text-base"
        >
          • 4 0 4 NO BUGS FOUND
        </motion.p>
        ...
        <motion.h1 variants={textVariant} className="mt-4 font-bold">
          ...
          <motion.span
            variants={textVariant}
            className="text-xl md:text-3xl font-bold bg-gradient-to-r from-[#64ffda] via-[#00fff0] to-[#00d9ff] bg-clip-text text-transparent drop-shadow-lg"
          >
        ...
        <motion.p
          variants={textVariant}
          className="mt-6 text-sm md:text-lg text-gray-200 leading-relaxed"
        >
...
>
  <ScrollLink
    to="portfolio"   // তোমার project section এর id="portfolio"
    smooth={true}
    duration={700}
    offset={-80} // navbar height বাদ দিতে
  >
  ...
  <a
    href="https://docs.google.com/document/d/1nT99iz9b839DubTinHg_vCOGnoaXAhW9llWDmtUyS1k/export?format=pdf"
    target="_blank"
  ...
  >
    <button className="px-6 py-3 rounded-full border border-gray-400 text-white font-semibold hover:bg-gray-800 hover:text-[#64ffda] transition-colors duration-300">
      ...
      <motion.div
        ...
        whileInView="visible"
        viewport={{ once: true }}
        className="relative z-10 mt-10 md:mt-0 flex justify-center"
      >
        {/* Glow Effect */}
        ...
        <motion.div
          ...
          className="absolute top-[-20px] left-[-20px] text-[#61dafb] text-3xl"
        >
          <FaReact />
        </motion.div>
        <motion.div
          variants={iconVariant}
          className="absolute top-[-10px] right-[-15px] text-[#f0db4f] text-3xl"
        >
          <FaJsSquare />
        </motion.div>
        <motion.div
          variants={iconVariant}
          className="absolute bottom-[-15px] left-[10px] text-[#3c873a] text-3xl"
        >
          <FaNodeJs />
        </motion.div>
        <motion.div
          variants={iconVariant}
          className="absolute bottom-[-10px] right-[10px] text-[#264de4] text-3xl"
        >
          <FaCss3Alt />
        </motion.div>
      </motion.div>
